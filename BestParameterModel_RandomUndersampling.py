...
model_name = 'simple_model_undersampling'
for i in range(5):
  ...
  min_len_resample = min(len(controls),len(right),len(left))
      
  right_resample = np.random.choice(range(len(right)),min_len_resample)
  left_resample = np.random.choice(range(len(left)),min_len_resample)
  controls_resample = np.random.choice(range(len(controls)),min_len_resample)
  
  print(right[1:5])
  print(right_resample[1:5])
  
  controls_undersample = []
  for i in controls_resample:
      np.random.seed(2)
      controls_undersample.append(controls[i])
  len(controls_undersample)
  
  right_undersample = []
  for i in right_resample:
      np.random.seed(2)
      right_undersample.append(right[i])
  len(right_undersample)

  left_undersample = []
  for i in left_resample:
      np.random.seed(2)
      left_undersample.append(left[i])
  len(left_undersample)
  
  
  train_patients_list_oversample = []
  train_patients_list_oversample = train_patients_list_oversample + controls_undersample + right_undersample + left_undersample
  np.random.seed(4)
